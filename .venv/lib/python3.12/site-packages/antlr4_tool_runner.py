...
def latest_version():
    try:
        ...
        version_dirs = list(
            filter(
                lambda directory: re.match(r"[0-9]+\.[0-9]+\.[0-9]+", directory),
                os.listdir(mvn_repo),
            )
        )
        ...
        if len(version_dirs) == 0:
            ...
            print(
                f"Found version '{latest_version_dir}', this version may be out of date"
            )
...
def download_antlr4(jar, version):
    ...
    try:
        with urlopen(
            f"https://repo1.maven.org/maven2/org/antlr/antlr4/{version}/antlr4-{version}-complete.jar",
            timeout=60,
        ) as response:
...
def install_jre(java_version="11"):
    ...
    r = input(
        f"ANTLR tool needs Java to run; install Java JRE 11 yes/no (default yes)? "
    )
